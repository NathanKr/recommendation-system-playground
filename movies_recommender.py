...
def compare_no_rating():
    v1 = np.where(Y.reshape(-1) == 0)
    v2 = np.where(R.reshape(-1) == 0)
...
R = mat_dict["R"] # R[i,j] is 1 if user j has rated movie i , R[i,j] is 0 otherwise
Y = mat_dict["Y"] # rating : 1,2,3,4,5 (0 relate to non rating , same as in R)
compare_no_rating()
n_m = Y.shape[0] # number of movies : 1682 -> num rows
n_u = Y.shape[1] # number of users : 943 -> num colmns
